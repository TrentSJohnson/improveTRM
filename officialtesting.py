...
class Testing:
        def test(self,algorithms, data, interval,cwgraph,start_time='started_at',epsilon=.1,radius=500):
            self.scores = []
            start_time = data[start_time][np.random.choice(data.index)]
            ...
            for i in tqdm(range(10)):
                ...
                while len(graph.nodes)<10:
                    start_time=data.starttime[np.random.choice(data.index)]
...
if __name__ == '__main__':
    ...
    filtered_data = data[(data[start_time]>=data.loc[0,start_time]) & (data[start_time]<=data.loc[0,start_time]+timedelta(minutes=15))]
